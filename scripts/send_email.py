#!/usr/bin/env python3
"""BillHub - 将 Docker 镜像分卷逐卷通过 SMTP 发送到指定邮箱。

只依赖 Python 标准库（smtplib + email.mime）。所有配置从环境变量读取，
由 GitHub Actions 从 Secrets 注入（SMTP_* 与 MAIL_TO 均在仓库 Secrets 配置），
仓库内不含任何明文凭证或收件地址。
...
环境变量：
    ...
    MAIL_TO       收件人地址（GitHub Secrets 配置，保证私密性）
    SPLIT_DIR     存放镜像分卷的目录（分卷名如 xxx.tar.zst.part001）
...
def collect_parts(split_dir):
    """收集分卷文件（xxx.tar.zst.part001/002/...），按名称排序。"""
    parts = sorted(p for p in glob.glob(os.path.join(split_dir, '*'))
                   if os.path.isfile(p))
    return parts
...
def send_part(cfg, path, index, total):
    ...
    body = (
        f'BillHub Docker 镜像分卷 第 {index}/{total} 卷\n'
        f'附件: {filename}\n\n'
        '请将全部卷收齐到同一目录后合并镜像：\n'
        '  Linux/macOS:  cat *.tar.zst.part* > billhub-web-<版本>.tar.zst\n'
        '  Windows:      copy /b *.part* billhub-web-<版本>.tar.zst\n'
        '再按 deploy.md 步骤导入：zstd -d 解压 → docker load → docker compose up -d。\n'
...
def main():
    ...
    if not parts:
        raise SystemExit(f'[ERROR] {cfg["split_dir"]} 下没有找到镜像分卷文件')
